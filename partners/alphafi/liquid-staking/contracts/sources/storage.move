module liquid_staking::storage {
    ...
    const EValidatorNotFound: u64 = 4;
    ...
    public(package) fun refresh<P>(
        ...
        while (i > 0) {
            ...
            if (self.validator_infos[i].inactive_stake.is_some()) {
                ...
                if (inactive_stake.stake_activation_epoch() <= ctx.epoch()) {
                    ...
                    self.join_inactive_stake_to_validator(i, inactive_stake);
                };
            };

        };

        if(!self.validator_addresses_and_weights.is_empty()) {
            self.rebalance<P>(system_state,ctx);
        } else {
            self.stake_pending_sui<P>(system_state,ctx);
        };
        true
    }

    public(package) fun stake_pending_sui<P>(
        self: &mut Storage, 
        system_state: &mut SuiSystemState, 
        ctx: &mut TxContext
    ): bool {
        ...
        while (i > 0) {
            ...
            self.increase_validator_stake<P>(
                system_state, 
                validator_address,
    ...
    public (package) fun set_validator_addresses_and_weights(
        self: &mut Storage,
        validator_addresses_and_weights: VecMap<address, u64>
    ) {
        self.version.assert_version_and_upgrade(CURRENT_VERSION);
    ...
    public (package) fun rebalance<P>(
        self: &mut Storage,
        system_state: &mut SuiSystemState,
        ctx: &mut TxContext
    ) {
        ...
        // No longer needed as this function will be called from liquid_staking_info
        //liquid_staking_info.refresh(system_state, ctx);

        let mut validator_addresses_and_weights = self.validator_addresses_and_weights;
        self.validator_addresses_and_weights = vec_map::empty<address,u64>();
        ...
        let validator_current_amounts = validator_addresses.map_ref!(|validator_address| {
            let validator_index = self.find_validator_index_by_address(*validator_address);
            if (validator_index >= self.validators().length()) {
                return 0
            };

            let validator = self.validators().borrow(validator_index);
        ...
        validator_addresses.length().do!(|i| {
            if (validator_current_amounts[i] > validator_target_amounts[i]) {
                self.decrease_validator_stake<P>(
                    system_state,
        ...
        while(i > 0) {
            i = i - 1;
            let (present, index) = validator_addresses.index_of(&(self.validator_infos[i].validator_address));
            if(present == true) {
                self.validator_infos[i].assigned_weight = validator_weights[index];
            }
        };
        self.last_refresh_epoch = ctx.epoch();
    }

    public (package) fun increase_validator_stake<P>(
        self: &mut Storage,
        system_state: &mut SuiSystemState,
        validator_address: address,
        sui_amount: u64,
        ...
        //Removing this ro stop recuserssion.
        //self.refresh<P>(system_state, ctx);

        let sui = self.split_up_to_n_sui_from_sui_pool(sui_amount);
            //    assert!(self.total_weight == self.total_sui_supply(),sui.value());
    ...
    public (package) fun decrease_validator_stake<P>(
        self: &mut Storage,
        system_state: &mut SuiSystemState,
        validator_address: address,
        ...
        //Removing this to stop recurssion. Storage already refreshed.
        //self.refresh<P>(system_state, ctx);

        let validator_index = self.find_validator_index_by_address(validator_address);
        assert!(validator_index < self.validators().length(), EValidatorNotFound);
        ...
        emit_event(DecreaseValidatorStakeEvent {
            typename: type_name::get<P>(),
            staking_pool_id: self.validators()[validator_index].staking_pool_id(),
            amount: sui_amount
        });
    ...
    public(package) fun split_n_sui(
...

        {
            ...
            while (i > 0 && self.sui_pool.value() < max_sui_amount_out) {
                ...
                let to_unstake_i = (((self.validator_infos[i].assigned_weight as u128 * (to_unstake as u128))
                                + total_weight - (1 as u128))/total_weight);
