module liquid_staking::fees {
    ...
    public struct FeeChangeEvent has copy, drop {
        typename: TypeName,
        sui_mint_fee_bps: u64,
        staked_sui_mint_fee_bps: u64, // unused
        redeem_fee_bps: u64,
    ...
    public fun set_flash_stake_fee_bps(mut self: FeeConfigBuilder, fee: u64): FeeConfigBuilder {
        bag::add(&mut self.fields, b"flash_stake_fee_bps", fee);
        self
    }
    ...
    public fun to_fee_config(builder: FeeConfigBuilder): FeeConfig {
        ...
        let fees = FeeConfig {
            ...
            flash_stake_fee_bps: if (bag::contains(&fields, b"flash_stake_fee_bps")) {
                bag::remove(&mut fields, b"flash_stake_fee_bps")
            } else {
                0
            },
    ...
    public(package) fun calculate_flash_stake_fee(self: &FeeConfig, sui_amount: u64): u64 {
        if (self.flash_stake_fee_bps == 0) {
            return 0
        };

        // ceil(sui_amount * redeem_fee_bps / 10_000)
        (((sui_amount as u128) * (self.flash_stake_fee_bps as u128) + 9999) / 10_000) as u64
    }

    public(package) fun emit_fee_change_event<P>(self: &FeeConfig) {
        emit_event(FeeChangeEvent {
            typename: type_name::get<P>(),
            sui_mint_fee_bps: self.sui_mint_fee_bps,
            staked_sui_mint_fee_bps: self.staked_sui_mint_fee_bps,   
            redeem_fee_bps: self.redeem_fee_bps,
            staked_sui_redeem_fee_bps: self.staked_sui_redeem_fee_bps,
            spread_fee_bps: self.spread_fee_bps,
            flash_stake_fee_bps: self.flash_stake_fee_bps,
            custom_redeem_fee_bps: self.custom_redeem_fee_bps,
            redeem_fee_distribution_component_bps: self.redeem_fee_distribution_component_bps
        });
    }
